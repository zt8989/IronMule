...
void CCorruptionBlackBox::TransferredData(uint64 nStartPos, uint64 nEndPos, const CUpDownClient* pSender){
	...
	if(pSender->GetIP().Type() != CAddress::IPv4) // IPv6-TODO: Add IPv6 ban list
		return;
	uint32 dwSenderIP = _ntohl(pSender->GetIP().ToIPv4());
#else // NEO: IP6 END
...
#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
				CUpDownClient* pEvilClient = theApp.clientlist->FindClientByIP(CAddress(_ntohl(aGuiltyClients[k])));
#else // NEO: IP6 END
...
#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
				CUpDownClient* pSuspectClient = theApp.clientlist->FindClientByIP(CAddress(_ntohl(aGuiltyClients[k])));
#else // NEO: IP6 END
