...
CFriend::CFriend(const uchar* abyUserhash, uint32 dwLastSeen, const CAddress& dwLastUsedIP, uint16 nLastUsedPort, 
...
void CFriend::FindKadID(){
		...
		&& GetLinkedClient()->GetKadPort() != 0 && GetLinkedClient()->GetKadVersion() >= 2)
	{
		...
		if(!GetLinkedClient()->GetIPv4().IsNull())
			Kademlia::CKademlia::FindNodeIDByIP(*this, _ntohl(GetLinkedClient()->GetIP().ToIPv4()), GetLinkedClient()->GetUserPort(), GetLinkedClient()->GetKadPort());
#else // NEO: IP6 END
