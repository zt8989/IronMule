...
void	CemuleApp::UpdateIPv6()
{
	...
	ULONG  flags = GAA_FLAG_INCLUDE_PREFIX | GAA_FLAG_SKIP_MULTICAST | GAA_FLAG_SKIP_DNS_SERVER | GAA_FLAG_INCLUDE_TUNNEL_BINDINGORDER;
	...
	do {
		...
		dwRetVal =
			GetAdaptersAddresses(AF_INET6/*AF_UNSPEC*/, flags, NULL, pAddresses, &outBufLen);
	...
	if (dwRetVal == NO_ERROR) {
        ...
        else
        {
            if (possibleAddresses.GetCount() > 1)
                IPv6 = possibleAddresses.GetTail();
