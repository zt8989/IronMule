...
//int CEncryptedDatagramSocket::DecryptReceivedClient(BYTE* pbyBufIn, int nBufLen, BYTE** ppbyBufOut, uconst CAddress& dwIP, uint16 nPort, uint32* nReceiverVerifyKey, uint32* nSenderVerifyKey, bool* bEncrypted) const{
//#else // NEO: IP6 END
//int CEncryptedDatagramSocket::DecryptReceivedClient(BYTE* pbyBufIn, int nBufLen, BYTE** ppbyBufOut, uint32 dwIP, uint16 nPort, uint32* nReceiverVerifyKey, uint32* nSenderVerifyKey, bool* bEncrypted) const{
//#endif
//#else // NEO: NAT END
#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
int CEncryptedDatagramSocket::DecryptReceivedClient(BYTE* pbyBufIn, int nBufLen, BYTE** ppbyBufOut, const CAddress& dwIP, uint32* nReceiverVerifyKey, uint32* nSenderVerifyKey) const{
#else // NEO: IP6 END
...
//			// ed2k packet from a client that does not know out userhash, but we know his
//			bKad = false;
//			bKadRecvKeyUsed = false;
//#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
//			uchar achKeyData[35];
//			int len;
//			if(byte* pTargetClientHash = theApp.clientudp->GetHashForEncryption(dwIP, nPort))
...
// else																-> ASSERT
#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
int CEncryptedDatagramSocket::EncryptSendClient(uchar** ppbyBuf, int nBufLen, const uchar* pachClientHashOrKadID, bool bKad, uint32 nReceiverVerifyKey, uint32 nSenderVerifyKey, bool bIPv6) const{
#else // NEO: IP6 END
