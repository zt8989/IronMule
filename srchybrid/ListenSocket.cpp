...
#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
						callback = theApp.clientlist->FindClientByIP(CAddress(ip), tcp);
#else // NEO: IP6 END
		...
		while (m_nPendingConnections > 0)
		{
...
#ifndef USE_IP_6 // IPv6-TODO: add this
			if (thePrefs.GetConditionalTCPAccept() && !thePrefs.GetProxySettings().UseProxy)
			{
...
#endif
			{
			    ...
			    if (theApp.clientlist->IsBannedClient(dwIP)){
				    if (thePrefs.GetLogBannedClients()){
#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
					    CUpDownClient* pClient = theApp.clientlist->FindClientByIP(CAddress(_ntohl(dwIP)));
#else // NEO: IP6 END
