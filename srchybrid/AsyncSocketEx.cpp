...
BOOL CAsyncSocketEx::Create(UINT nSocketPort /*=0*/, int nSocketType /*=SOCK_STREAM*/, 
							...
							LPCSTR lpszSocketAddress /*=NULL*/, BOOL bReuseAddr /*=FALSE*/, BOOL bIPv6 /*= FALSE*/ )
#else // NEO: IP6 END
BOOL CAsyncSocketEx::Create(UINT nSocketPort /*=0*/, int nSocketType /*=SOCK_STREAM*/, 
							long lEvent /*=FD_READ | FD_WRITE | FD_OOB | FD_ACCEPT | FD_CONNECT | FD_CLOSE*/, 
...
BOOL CAsyncSocketEx::Bind(UINT nSocketPort, LPCSTR lpszSocketAddress)
{
	...
	if(m_SocketData.bIPv6)
	{
		...
		else 
		{
			...
			int sslen = sizeof(ss);
			if(WSAStringToAddressA((char*)lpszSocketAddress, AF_INET6, NULL, (struct sockaddr*)&ss, &sslen) == 0)
...
BOOL CAsyncSocketEx::Connect(LPCSTR lpszHostAddress, UINT nHostPort)
{
...
#endif //NOLAYERS
	{
		...
		if(m_SocketData.bIPv6)
		{
			...
			struct sockaddr_storage ss;
			int sslen = sizeof(ss);
			if(WSAStringToAddressA((char*)lpszHostAddress, AF_INET6, NULL, (struct sockaddr*)&ss, &sslen) == 0)
				sockAddr.sin6_addr = ((struct sockaddr_in6 *)&ss)->sin6_addr;
			else
			{
				WSASetLastError(WSAEINVAL);
