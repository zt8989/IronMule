...
#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
float CClientCredits::GetScoreRatio(const CAddress& dwForIP) const
#else // NEO: IP6 END
...
uint8 CClientCreditsList::CreateSignature(CClientCredits* pTarget, uchar* pachOutput, uint8 nMaxSize, 
										  const CAddress& ChallengeIP, uint8 byChaIPKind, 
...
#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
						ChallengeIP = CAddress(_ntohl(theApp.serverconnect->GetLocalIP()));
#else // NEO: IP6 END
...
#endif
#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
uint32 CClientCredits::GetSecureWaitStartTime(const CAddress& dwForIP)
#else // NEO: IP6 END
