...
BOOL CClientDetailPage::OnInitDialog()
{
	...
	AddAnchor(IDC_USE_IPV6, TOP_LEFT, TOP_RIGHT);
	AddAnchor(IDC_CLIENT_IPV6, TOP_LEFT, TOP_RIGHT);
...
BOOL CClientDetailPage::OnSetActive()
{
	...
	if (m_bDataChanged)
	{
		...
		*/
		...
		GetDlgItem(IDC_USE_IPV6)->SetWindowText(client->IsIPv6Open()?_T("Yes"):_T("No"));

		GetDlgItem(IDC_CLIENT_IPV6)->SetWindowText(client->IsIPv6Open()?ipstr(client->GetIPv6()):_T(""));
