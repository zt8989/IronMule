...
CFriend* CFriendList::SearchFriend(const uchar* abyUserHash, const CAddress& IP, uint16 nPort) const {
#else // NEO: IP6 END
...
#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
			if (cur_friend->m_dwLastUsedIP == IP && !IP.IsNull() && cur_friend->m_nLastUsedPort == nPort && nPort != 0)
#else // NEO: IP6 END
