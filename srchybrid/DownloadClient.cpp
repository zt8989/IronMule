...
bool CUpDownClient::Compare(const CUpDownClient* tocomp, bool bIgnoreUserhash) const
{
...
#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
		if ((!GetIPv4().IsNull() && !tocomp->GetIPv4().IsNull()) || (!GetIPv6().IsNull() && !tocomp->GetIPv6().IsNull()))
#else // NEO: IP6 END
	...
	if(GetKadPort()!=0)
	{
...
#endif
		{
			//Both clients have a verified IP.
#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
			if((GetIP() == tocomp->GetIP() || GetIPv6() == tocomp->GetIPv6()) && GetKadPort() == tocomp->GetKadPort())
#else // NEO: IP6 END
	...
	if (SupportMultiPacket() || SupportsFileIdentifiers())
	{
		...
		if (IsSourceRequestAllowed())
		{
			...
			if (SupportsSourceExchange2()){
				...
				dataFileReq.WriteUInt8(SupportsSourceExchangeExt() ? SOURCEEXCHANGEEXT_VERSION: SOURCEEXCHANGE2_VERSION);
#else // NEO: NSX END
...
#ifdef USE_NEO_SX // NEO: NSX - [NeoSoruceExchange]
				PokeUInt8(&packet->pBuffer[0], SupportsSourceExchangeExt() ? SOURCEEXCHANGEEXT_VERSION: SOURCEEXCHANGE2_VERSION);
#else // NEO: NSX END
