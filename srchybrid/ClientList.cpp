...
CUpDownClient* CClientList::FindClientByIP(const CAddress& IP, UINT port) const
#else // NEO: IP6 END
...
#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
		if ((IP.Type() == CAddress::IPv6 ? IP == cur_client->GetIPv6() : IP == cur_client->GetIP()) && cur_client->GetUserPort() == port)
#else // NEO: IP6 END
...
#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
CUpDownClient* CClientList::FindClientByUserHash(const uchar* clienthash, const CAddress& IP, uint16 nTCPPort) const
#else // NEO: IP6 END
...
#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
			if ((IP.IsNull() || (IP.Type() == CAddress::IPv6 ? IP == cur_client->GetIPv6() : IP == cur_client->GetIP())) && (nTCPPort == 0 || nTCPPort == cur_client->GetUserPort())) 
#else // NEO: IP6 END
...
#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
CUpDownClient* CClientList::FindClientByIP(const CAddress& IP) const
#else // NEO: IP6 END
...
#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
CUpDownClient* CClientList::FindClientByIP_UDP(const CAddress& IP, UINT nUDPport) const
#else // NEO: IP6 END
...
#endif
{
	for (POSITION pos = list.GetHeadPosition(); pos != NULL;)
	{
		CUpDownClient* cur_client = list.GetNext(pos);
#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
		if ((IP.Type() == CAddress::IPv6 ? IP == cur_client->GetIPv6() : IP == cur_client->GetIP()) && cur_client->GetUDPPort() == nUDPport)
#else // NEO: IP6 END
...
#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
CUpDownClient* CClientList::FindClientByIP_KadPort(const CAddress& IP, uint16 port) const
#else // NEO: IP6 END
...
#endif
{
	for (POSITION pos = list.GetHeadPosition(); pos != NULL;)
	{
		CUpDownClient* cur_client = list.GetNext(pos);
#ifdef USE_IP_6 // NEO: IP6 - [IPv6]
		if ((IP.Type() == CAddress::IPv6 ? IP == cur_client->GetIPv6() : IP == cur_client->GetIP()) && cur_client->GetKadPort() == port)
#else // NEO: IP6 END
...
void CClientList::AddTrackClient(CUpDownClient* toadd){
	...
	if(toadd->GetIP().Type() != CAddress::IPv4) // IPv6-TODO: Add IPv6 tracking
		return;
	uint32 dwIP = _ntohl(toadd->GetIP().ToIPv4());
#else // NEO: IP6 END
	uint32 dwIP = toadd->GetIP();
#endif
...
void CClientList::TrackBadRequest(const CUpDownClient* upcClient, int nIncreaseCounter){
	...
	if(upcClient->GetIP().Type() != CAddress::IPv4) // IPv6-TODO: Add IPv6 tracking
		return;
	uint32 dwIP = _ntohl(upcClient->GetIP().ToIPv4());
#else // NEO: IP6 END
	uint32 dwIP = upcClient->GetIP();
#endif
